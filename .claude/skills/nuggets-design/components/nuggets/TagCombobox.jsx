...
export function TagCombobox({ value = [], options = [], onChange, label = 'Tags', placeholder = 'Add a tag…', style }) {
  ...
  const rows = isNew ? [...matches, { create: norm }] : matches;

  const add = (name) => { const n = normalise(name); if (n && !value.includes(n)) onChange && onChange([...value, n]); setQ(''); setCursor(0); };
  const key = (e) => {
