...
export function Wordmark({ size = 24, tone = 'ink', withMark = true, iconOnly = false, style }: WordmarkProps) {
  ...
  if (iconOnly) {
    return <img src={nuggetSvg} alt="Nuggets" style={{ width: size, height: size * 0.83, display: 'block', ...style }} />;
  }
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: size * 0.3, ...style }}>
      {withMark && <img src={nuggetSvg} alt="" aria-hidden="true" style={{ width: size * 1.15, height: size * 0.96, display: 'block' }} />}
